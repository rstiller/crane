...
function Service() {
    ...
    this.buildPuppetFacts = function(dockerfile, files) {
        if(!!slf.provision.facts) {
            for(var i = 0; i < slf.provision.facts.length; i++) {
                files.push(slf.provision.facts[i]);
                dockerfile.push('ADD ' + slf.provision.facts[i] + ' /etc/facter/facts.d/');
            }
        }
    };
    
    this.buildPuppetModules = function(dockerfile, modules, files) {
        var index = 0;
        
        if(!!slf.provision.modulePaths) {
            for(var i = 0; i < slf.provision.modulePaths.length; i++) {
                var path = '/tmp/puppet/_modules-' + index++ + '/';
                files.push(slf.provision.modulePaths[i]);
                dockerfile.push('ADD ' + slf.provision.modulePaths[i] + ' ' + path);
            }
        }
    };
    
    this.buildPuppetManifest = function(dockerfile, files) {
        files.push(slf.provision.manifest);
        dockerfile.push('ADD ' + slf.provision.manifest + ' /tmp/puppet/_manifest/manifest.pp');
    };
    
    this.buildPuppetProvisioning = function(dockerfile, files) {
        var modules = [];
        
        slf.buildPuppetFacts(dockerfile, files);
        slf.buildPuppetModules(dockerfile, modules, files);
        slf.buildPuppetManifest(dockerfile, files);
    ...
    this.buildShellProvisioning = function(dockerfile, files) {
    ...
    this.buildProvisioning = function(dockerfile, files) {
        if(!!slf.provision) {
            if(slf.provision.provider === 'puppet') {
                str = slf.buildPuppetProvisioning(dockerfile, files);
            } else if(slf.provision.provider === 'shell') {
                str = slf.buildShellProvisioning(dockerfile, files);
            }
        }
    };
    ...
    this.buildDockerfile = function(options) {
        ...
        slf.buildProvisioning(dockerfile, files);
