...
function RevisionChecker() {
    ...
    slf.emitter.on('build', function(project, branches, tags) {
        ...
        var buildInfrastructure = function(workingCopies, type, name, rev, next) {
            ...
            async.waterfall([
                ...
                function(files, ready) {
                    infrastructure.files = infrastructure.files || {};
                    var fileContents = [];
                    
                    _.each(files, function(hash, name) {
                        fileContents.push(function(nextFile) {
                            repository.readFile(hash, function(content) {
                                if(!!name && name != '') {
                                    infrastructure.files[name] = new Buffer(content).toString('base64');
                                }
                                nextFile(null);
                            });
                        });
                ...
                function(ready) {
                    ...
                    _.each(infrastructure.environments, function(variables, environment) {
                        _.each(infrastructure.services, function(config, service) {
                            var build = new ImageBuildCommand(project._id, name, type, service, environment);
                    ...
                    DBS.Builds.bulkDocs({
                        ...
                        LOG.info('build commands submitted for project %s - %s %s', project.name, name, rev);
        ...
        _.each(branches, function(rev, name) {
            funcs.push(function(next) {
                buildInfrastructure(project.branches, 'branch', name, rev, next);
            });
        });
        
        _.each(tags, function(rev, name) {
            funcs.push(function(next) {
                buildInfrastructure(project.tags, 'tag', name, rev, next);
            });
        });
