...
module.exports.RevisionCheckTask = Backbone.Model.extend({
    ...
    saveBuildCommands: function(next) {
        var docs = [];
        var slf = this;
        ...
        BuildCommand.saveAll(docs, function(err) {
            LOG.info(__filename + ' - build commands submitted for project %s (name: %s, rev: %s)', slf.get('project').get('name'), slf.get('name'), slf.get('rev'));
            next(err);
        });
    },
    getExecutionChain: function() {
        ...
        return [
            ...
            function(next) { slf.removeWorkingCopy(next); },
            function(next) { slf.saveBuildCommands(next); }
        ];
    },
